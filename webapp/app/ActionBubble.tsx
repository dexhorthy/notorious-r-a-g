...
export default function ActionBubble({ action, startDate }: ActionBubbleProps) {
  ...
  return (
    <div
      className={`rounded-lg flex flex-row gap-2 items-center p-3 mb-2 ${actionConfig.class}`}
    >
        ...
        content.result.map(({ content, metadata }, index) => {
          ...
          if (thread_id && channel_id) {
            ...
            if (channel_id in channelNames) {
              restMeta["channel_name"] =
                channelNames[channel_id as keyof typeof channelNames]
            }
            restMeta[
              "url"
            ] = `https://discord.com/channels/1119368998161752075/${channel_id}/${thread_id}`
          }
          meta = restMeta
...
function FullContent({ content }: { content: string }) {
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  return (
    <>
      <button
      ...
      <Sheet open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <SheetContent className="w-[800px] bg-gray-300">
          <SheetTitle>Full Content</SheetTitle>
